...
// 메모 단건 조회 (GET)
export async function GET(req: NextRequest, { params }: { params: { memoId: string } }) {
    ...
    try {
        const response = await fetch(
            `${API_BASE_URL}/projects/${projectId}/file/${fileId}/memos/${memoId}`
        );
...
// 메모 수정 (PATCH)
export async function PATCH(req: NextRequest, { params }: { params: { memoId: string } }) {
    const { searchParams } = new URL(req.url);
    const projectId = searchParams.get("projectId");
    const fileId = searchParams.get("fileId");
    const { memoId } = params;
...
// 메모 삭제 (DELETE)
export async function DELETE(req: NextRequest, { params }: { params: { memoId: string } }) {
    const { searchParams } = new URL(req.url);
    const projectId = searchParams.get("projectId");
    const fileId = searchParams.get("fileId");
    const { memoId } = params;
